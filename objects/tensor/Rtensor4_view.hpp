...
namespace cnine{
  ...
  public: // ---- Other views -------------------------------------------------------------------------------
    ...
    Rtensor3_view slice0(const int i){
      return Rtensor3_view(arr+i*s0,n1,n2,n3,s1,s2,s3);
    }

    Rtensor3_view slice1(const int i){
      return Rtensor3_view(arr+i*s1,n0,n2,n3,s0,s2,s3);
    }

    Rtensor3_view slice2(const int i){
      return Rtensor3_view(arr+i*s2,n0,n1,n3,s0,s1,s3);
    }

    Rtensor3_view slice3(const int i){
      return Rtensor3_view(arr+i*s3,n0,n1,n2,s0,s1,s2);
    }

    Rtensor3_view fuse01(){
      return Rtensor3_view(arr,n0*n1,n2,n3,s1,s2,s3);
    }    

    Rtensor3_view fuse12(){
      return Rtensor3_view(arr,n0,n1*n2,n3,s0,s2,s3);
    }    

    Rtensor3_view fuse23(){
      return Rtensor3_view(arr,n0,n1,n2*n3,s0,s1,s3);
    }    



    Rtensor2_view slice01(const int i, const int j){
      return Rtensor2_view(arr+i*s0+j*s1,n2,n3,s2,s3);
    }

    Rtensor2_view slice02(const int i, const int j){
      return Rtensor2_view(arr+i*s0+j*s2,n1,n3,s1,s3);
    }

    Rtensor2_view slice03(const int i, const int j){
      return Rtensor2_view(arr+i*s0+j*s3,n1,n2,s1,s2);
    }

    Rtensor2_view slice12(const int i, const int j){
      return Rtensor2_view(arr+i*s1+j*s2,n0,n3,s0,s3);
    }

    Rtensor2_view slice13(const int i, const int j){
      return Rtensor2_view(arr+i*s1+j*s3,n0,n2,s0,s2);
    }

    Rtensor2_view slice23(const int i, const int j){
      return Rtensor2_view(arr+i*s2+j*s3,n0,n1,s0,s1);
    }
  ...
  inline Rtensor4_view split0(const Rtensor3_view& x, const int i, const int j){
      assert(i*j==x.n0);
      return Rtensor4_view(x.arr,i,j,x.n1,x.n2,x.s0*j,x.s0,x.s1,x.s2);
  }

  inline Rtensor4_view split1(const Rtensor3_view& x, const int i, const int j){
      assert(i*j==x.n1);
      return Rtensor4_view(x.arr,x.n0,i,j,x.n2,x.s0,x.s1*j,x.s1,x.s2);
    }

  inline Rtensor4_view split2(const Rtensor3_view& x, const int i, const int j){
    assert(i*j==x.n2);
    return Rtensor4_view(x.arr,x.n0,x.n1,i,j,x.s0,x.s1,x.s2*j,x.s2);
  }
