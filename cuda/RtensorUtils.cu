...
__global__ void GivensSomeSlices_kernel_t(float* arr, const int s0, const int s1, const int* ix, const float* c){
...
__global__ void GivensSomeSlices_kernel_tt(float* arr, const int s0, const int s1, const int s2, const int* ix, const float* c){
...
__global__ void GivensSomeSlices_kernel_bt(float* arr, const int s0, const int s1, const int s2, const int* ix, const float* c){
...
namespace cnine{
  ...
  void ScaleSomeSlices_cu(const Rtensor3_view& x, const Itensor1_view& indices, const Rtensor1_view& coeffs, const cudaStream_t& stream){
    ...
    if(x.n1*x.n2<=1024){
      ...
      ScaleSomeSlices_kernel_t<<<N,threads,0,stream>>>(x.arr,x.s0,x.s1,x.s2,indices.arr,coeffs.arr);
      return;
    }
    ...
    if(x.n1<=1024||x.n2<=1024){
      if(x.n1%32>x.n2%32){
	dim3 blocks(N,x.n1);
	ScaleSomeSlices_kernel_t<<<blocks,x.n2,0,stream>>>(x.arr,x.s0,x.s1,x.s2,indices.arr,coeffs.arr);
      }else{
	dim3 blocks(N,x.n2);
	ScaleSomeSlices_kernel_t<<<blocks,x.n2,0,stream>>>(x.arr,x.s0,x.s2,x.s1,indices.arr,coeffs.arr);
      }
    }
  ...
  void GivensSomeSlices_cu(const Rtensor3_view& x, const Itensor2_view& indices, const Rtensor2_view& coeffs, const cudaStream_t& stream){
    ...
    if(x.n1*x.n2<=1024){
      dim3 threads(x.n1,x.n2);
      GivensSomeSlices_kernel_t<<<N,threads,0,stream>>>(x.arr,x.s0,x.s1,x.s2,indices.arr,coeffs.arr);
      return;
    }
    ...
    if(x.n1<=1024||x.n2<=1024){
      if(x.n1%32>x.n2%32){
	dim3 blocks(N,x.n1);
	GivensSomeSlices_kernel_t<<<blocks,x.n2,0,stream>>>(x.arr,x.s0,x.s1,x.s2,indices.arr,coeffs.arr);
      }else{
	dim3 blocks(N,x.n2);
	GivensSomeSlices_kernel_t<<<blocks,x.n2,0,stream>>>(x.arr,x.s0,x.s2,x.s1,indices.arr,coeffs.arr);
      }
    }
