...
namespace cnine{ 
  ...
  inline void dispatch(const Rtensor3_view& r, const Rtensor3_view* x, 
    ...
    if(n2<=1024){
      dim3 blocks(n0,n1);
      dim3 threads(n2);
      bbt_fn(blocks,threads,x.s0,x.s1,x.s2,t.s0,r.s0,r.s2);
      return;
    }
    ...
    if(n1<=1024){
      dim3 blocks(n0,n2);
      dim3 threads(n1);
      bbt_fn(blocks,threads,x.s1,x.s0,x.s2,t.s1,r.s0,r.s2);
      return;
    }
    ...
    if(n0<=1024){
      dim3 blocks(n1,n2);
      dim3 threads(n0);
      bbt_fn(blocks,threads,x.s2,x.s0,x.s1,t.s2,r.s0,r.s1);
      return;
    }
