...
namespace cnine{
  ...
  template<typename TYPE>
  void TensorView_add_cu(const Ltensor<TYPE>& r, const Ltensor<TYPE>& x, const cudaStream_t& stream){
    CNINE_ASSRT(r.get_dims()==x.get_dims());
    int D=r.ndims();

    if(D==1){
      if(r.dim[0]>1024)
	TensorView_add_kernel_bt<<<r.dim[0]/1024,1024,0,stream>>>(r.get_arr(),x.get_arr(),1024*r.strides[0],r.strides[0],1024*x.strides[0],x.strides[0]);
      if(r.dim[0]%1024>0)
	TensorView_add_kernel_t<<<1,r.dim[0]%1024,0,stream>>>(r.get_arr(),x.get_arr(),r.strides[0],x.strides[0]);
    }
	...
	return;
      ...
      if(R.dim[1]<=1024){
	TensorView_add_kernel_bt<<<r.dim[0],r.dim[1],0,stream>>>(r.get_arr(),x.get_arr(),r.strides[0],r.strides[1],x.strides[0],x.strides[1]);
	return;
      }
      ...
      if(r.dim[1]%1024>0)
	TensorView_add_kernel_bt<<<r.dim[0],r.dim[1]%1024,0,stream>>>(r.get_arr(),x.get_arr(),r.strides[0],r.strides[1],x.strides[0],x.strides[1]);
	...
	TensorView_add_kernel_btt<<<R.dim[0],threads,0,stream>>>(r.get_arr(),x.get_arr(),r.strides[0],r.strides[1],r.strides[2],x.strides[0],x.strides[1],x.strides[2]);
	return;
      }

      if(r.dim[2]<=1024){
	dim3 blocks(r.dim[0],r.dim[1]);
	TensorView_add_kernel_bbt<<<blocks,r.dim[2],0,stream>>>(r.get_arr(),x.get_arr(),r.strides[0],r.strides[1],r.strides[2],x.strides[0],x.strides[1],x.strides[2]);
	return;
      }
	...
	TensorView_add_kernel_bbt<<<blocks2,r.dim[2]%1024,0,stream>>>(r.get_arr(),x.get_arr(),r.strides[0],r.strides[1],r.strides[2],x.strides[0],x.strides[1],x.strides[2]);
      }
